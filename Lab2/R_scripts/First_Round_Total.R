...
#based on CPI.
tmp=quantile(benchmark[,8])
zero_quartile=tmp[1]
first_quartile=tmp[2] 
aux1 = which(benchmark[,8] < first_quartile)
aux2 = which(benchmark[,8] == zero_quartile)
...
png(sprintf("Total_Round1_NEW_%s.png",titles[i]), width=3.5, height=3.25, 
    units="in", res=1200)
...
for (j in seq(6)){
  ...
  png(sprintf("Best_Round1_NEW_%s.png",titles[j]), width=3.5, height=3.25, 
      units="in", res=1200)
  ylim <- c(0, 1.3*max(absfreq$Freq))
  xx <- barplot(absfreq$Freq, xaxt = 'n', xlab = x_label[j%/%4 +1], width = 0.85,
